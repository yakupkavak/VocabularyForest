...
extension Forest: ConvertSafeModel {
    ...
    func safeObject() throws -> SafeForestModel {
        
        let safeQuests: [QuestModel]
        if let questSet = self.quests as? Set<Quest> {
            safeQuests = try questSet.map { try $0.safeObject() }
        } else {
            safeQuests = []
        }
        ...
        if let sculptureSet = self.sculptures as? Set<Sculpture> {
            safeSculptures = try sculptureSet.map { try $0.safeObject() }
        } else {
            safeSculptures = []
        }
        ...
        if let treeSet = self.trees as? Set<Tree> {
            safeTrees = try treeSet.map { try $0.safeObject() }
        } else {
            safeTrees = []
        }
        ...
        if let animalSet = self.animals as? Set<Animal> {
            safeAnimals = try animalSet.map { try $0.safeObject() }
        } else {
            safeAnimals = []
        }
